...
permanova_aitch <- adonis2(t(assay(tse_baseline, "clr")) ~  Gruppe+ BMI + Alter + Geschlecht + Observed + Prevotella_genus + Bacteroides_genus + Ruminococcus_genus,
                         by = "margin", # each term analyzed individually
                         data = colData(tse_baseline),
                         method = "euclidean",
                         permutations = 9999, 
                         na.action=na.omit)
permanova_aitch_df <- as.data.frame(permanova_aitch)


df <- data.frame(permanova_euc_df[,"Pr(>F)", drop=F],
                 permanova_aitch[,"Pr(>F)", drop=F],
                 permanova_bray_df[,"Pr(>F)", drop=F])
colnames(df) <- c("euclidean", "Aitchison", "bray")
df
```  

```
                   euclidean Aitchison   bray
Gruppe                0.1811    0.5579 0.2791
BMI                   0.8870    0.0388 0.5102
Alter                 0.3989    0.2974 0.1634
Geschlecht            0.0737    0.2474 0.1034
Observed              0.0001    0.0001 0.0001
Prevotella_genus      0.0001    0.0225 0.0001
Bacteroides_genus     0.0001    0.0482 0.0001
Ruminococcus_genus    0.0002    0.1220 0.0001
Residual                  NA        NA     NA
Total                     NA        NA     NA
...
permanova_aitch_df
                    Df SumOfSqs      R2      F Pr(>F)    
Gruppe               1     6377 0.00837 0.9514 0.5579    
BMI                  1     8940 0.01174 1.3338 0.0388 *  
Alter                1     7017 0.00921 1.0468 0.2974    
Geschlecht           1     7213 0.00947 1.0761 0.2474    
Observed             1    37265 0.04892 5.5597 0.0001 ***
Prevotella_genus     1     9785 0.01285 1.4599 0.0225 *  
Bacteroides_genus    1     8803 0.01156 1.3133 0.0482 *  
Ruminococcus_genus   1     7899 0.01037 1.1784 0.1220    
Residual            97   650161 0.85352                  
Total              105   761741 1.00000                  
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
```
