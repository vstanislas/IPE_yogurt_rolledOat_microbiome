...
# 
# library(ggnewscale)
# plot_sample_measure <- function(obj, measure, subtitle="", x= "Treatment", colorInter=F, scale_log =F, 
#                                 print_table=T, lineMean=F, printMean=F, ind_filter=NULL, line_diff=T, 
#                                 linewidth_diff=1.4, ylim=NULL, counts=F){
#   
#   
...
#   
#   ## Calculate difference between timepoint 
...
#       geom_line(aes(group=Participant_ID, color=Diff), linewidth=linewidth_diff, alpha=0.2) + 
...
#       
#       pp <- pp + scale_fill_manual(values= cols_Inter_treat)  
#     }  
#     else {
...
#   } else {
#     if(colorInter)  {
#       pp <- pp + geom_jitter(aes(color=Inter_treat), width = 0.2, size=1.2, alpha=0.7)  +
#         scale_colour_manual(values= cols_Inter_treat) 
#     }
#     else  pp <- pp + geom_jitter(width = 0.2, size=1.2, alpha=0.7) 
#     pp <- pp+ 
#       geom_violin(color="gray70", linewidth=0.4, width= 0.6, alpha=0)   + 
...
#   
#   
#   if(x=="Treatment") pp <-  pp  + facet_grid( ~ Intervention) 
#   if(x=="Timepoint") pp <-  pp  + facet_grid( ~ Gruppe ) 
#   
#   if(scale_log){
#     if(printMean | lineMean) pp <- pp + coord_trans(y = "pseudo_log", ylim=ylim) # compute stat before axis transformation
#     else pp <- pp + scale_y_continuous(trans=scales::pseudo_log_trans(base = 10), limits= ylim) # compute stat AFTER axis transformation
#   }   
#   
#   if(print_table)  pp <- pp + tableGrob(table(DF$Diff, DF$Intervention)/2, theme=ttheme_default(base_size =7)) + 
