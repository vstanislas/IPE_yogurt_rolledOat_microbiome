...
## COVID STUDY
id_IS <- grepl("IS", colnames(coverage), fixed = TRUE)
id_IS[1] <- TRUE
coverage_IS <- coverage[, id_IS]
coverage_IS <- coverage_IS[order(coverage_IS$Pathway), ]

## separate pathway info in categories
coverage_IS <- separate(coverage_IS, c("Pathway", "Taxonomy"), col="Pathway", sep="\\|")
coverage_IS <- separate(coverage_IS, c("Pathway", "Descr"), col="Pathway", sep=":")

## remove empty rows
dim(coverage_IS) # 27630   268
rs <- rowSums(coverage_IS[, 4:ncol(coverage_IS)], na.rm =T) 
# ess <- coverage_IS[which(rs==0), ]
# dim(ess) # 5405  268
# sum(ess[, 4:ncol(ess)], na.rm=T) # 0
# sum(coverage_IS[, 4:ncol(coverage_IS)], na.rm=T) #  279856
coverage_IS <- coverage_IS[which(rs>0), ] 
# sum(coverage_IS[, 4:ncol(coverage_IS)], na.rm=T) # 279856
# dim(coverage_IS) # 22225   268

save(coverage_IS, file="Q:/IPE-P-COVID-19-Impfstudie/Analysis/DataPreparation/Metagenomic/Microbiome_DATA_Kun_Huang/Functional/IS_coverage.Rdata")
...
## COVID STUDY
id_IS <- grepl("IS", colnames(pathway), fixed = TRUE)
id_IS[1] <- TRUE
pathway_IS <- pathway[, id_IS]
pathway_IS <- pathway_IS[order(pathway_IS$Pathway), ]

## separate pathway info in categories
pathway_IS <- separate(pathway_IS, c("Pathway", "Taxonomy"), col="Pathway", sep="\\|")
pathway_IS <- separate(pathway_IS, c("Pathway", "Descr"), col="Pathway", sep=":")


## remove empty rows
dim(pathway_IS) #  27630   268
rs <- rowSums(pathway_IS[, 4:ncol(pathway_IS)], na.rm =T) 
# ess <- pathway_IS[which(rs==0), ]
# dim(ess) # 4419  268
# sum(ess[, 4:ncol(ess)], na.rm=T) # 0
# sum(pathway_IS[, 4:ncol(pathway_IS)], na.rm=T) #  443505025
pathway_IS <- pathway_IS[which(rs>0), ] 
# sum(pathway_IS[, 4:ncol(pathway_IS)], na.rm=T) # 443505025
# dim(pathway_IS) # 23211   268

save(pathway_IS, file="Q:/IPE-P-COVID-19-Impfstudie/Analysis/DataPreparation/Metagenomic/Microbiome_DATA_Kun_Huang/Functional/IS_pathway.Rdata")
...
## COVID STUDY
id_IS <- grepl("IS", colnames(unpck_pathway), fixed = TRUE)
id_IS[1] <- TRUE
unpck_pathway_IS <- unpck_pathway[, id_IS]
unpck_pathway_IS <- unpck_pathway_IS[order(unpck_pathway_IS$Pathway), ]


## separate pathway info in categories
unpck_pathway_IS <- separate(unpck_pathway_IS, c("Pathway", "Taxonomy", "Uniref"), col="Pathway", sep="\\|")
unpck_pathway_IS <- separate(unpck_pathway_IS, c("Pathway", "Descr"), col="Pathway", sep=":")


## remove empty rows
dim(unpck_pathway_IS) # 1355892     269   
rs <- rowSums(unpck_pathway_IS[, 5:ncol(unpck_pathway_IS)], na.rm =T) 
# ess <- unpck_pathway_IS[which(rs==0), ]
# dim(ess) # 272411    269
# sum(ess[, 5:ncol(ess)], na.rm=T) # 0
# sum(unpck_pathway_IS[, 5:ncol(unpck_pathway_IS)], na.rm=T) #  466519272
unpck_pathway_IS <- unpck_pathway_IS[which(rs>0), ] 
# sum(unpck_pathway_IS[, 5:ncol(unpck_pathway_IS)], na.rm=T) # 466519272
# dim(unpck_pathway_IS) # 1083481     269

save(unpck_pathway_IS, file="Q:/IPE-P-COVID-19-Impfstudie/Analysis/DataPreparation/Metagenomic/Microbiome_DATA_Kun_Huang/Functional/IS_unpck_pathway.Rdata")





ss <- subset(pathway_det, Pathway=="1CMET2-PWY" & Taxonomy=="g__Blautia.s__Ruminococcus_torques")
sum(ss$JH001_Abundance.CPM[-1], na.rm=T)
ss[1:5,1:5]
