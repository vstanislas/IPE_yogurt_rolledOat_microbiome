...
summary(colSums(KEGG_JH))
 ...
 # 996966  999783  999833  999804  999867  999976 
```
```{r}
...
```{r}
inters_names <- intersect(colnames(KEGG_JH), rownames(metadata))
cat("Samples present in HUMAnN and absent from Metadata: ")
colnames(KEGG_JH)[!(colnames(KEGG_JH) %in% inters_names)]
...
```{r}
save(tse_KEGG, physeq_KEGG, file=paste(dir1, "Functional/TSE_Physeq_KEGG.Rdata", sep=""))
```
...
write.xlsx(desc_df_toSave, file=paste(dir, "DataAnalysis/KEGG_description.xlsx", sep=""), row.names = F)
```
