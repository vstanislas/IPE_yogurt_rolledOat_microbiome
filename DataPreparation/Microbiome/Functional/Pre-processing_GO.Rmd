...
```{r}
inters_names <- intersect(colnames(GO_JH), rownames(metadata))
cat("Samples present in HUMAnN and absent from Metadata: ")
colnames(GO_JH)[!(colnames(GO_JH) %in% inters_names)]
...
```{r}
save(tse_GO, physeq_GO, file=paste(dir1, "Functional/TSE_Physeq_GO.Rdata", sep=""))
```
...
write.xlsx(desc_df_toSave, file=paste(dir, "DataAnalysis/GO_description.xlsx", sep=""), row.names = F)
```
