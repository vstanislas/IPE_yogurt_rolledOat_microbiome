---
title: "Analysis of functional 946461 Genes from CPM table"
output: 
  html_notebook:
    number_sections: yes
    toc: yes
...
path_analysis <- paste(dir, "DataAnalysis/Results_Main_Analysis/ResultsFunctional/On_946461genes_fromCPM/", sep="")
...
# Data preparation
```{r message=FALSE}
load(paste(dir, "DataPreparation/Microbiome/Functional/TSE_Physeq_946461genes_fromCPM.Rdata", sep=""))
...
### UNMAPPED explain all the variation on first axis
...
PRS_all_full <- read.table(file=paste(path, "NBZIMM_asinsqrt_full_PRS_all_results.tsv", sep=""))
PRS_all_Y <- read.table(file=paste(path, "NBZIMM_asinsqrt_Y_PRS_all_results.tsv", sep=""))
PRS_all_YO <- read.table(file=paste(path, "NBZIMM_asinsqrt_YO_PRS_all_results.tsv", sep=""))
```
...
genes_sig_full <- PRS_all_full_sig$feature
genes_sig_Y <- PRS_all_Y_sig$feature
genes_sig_YO <- PRS_all_YO_sig$feature
```


```{r}
nrow(PRS_all_full_sig) # 7/946460*100= 0
nrow(PRS_all_Y_sig) # 3503/946460*100= 0.37
nrow(PRS_all_YO_sig) # 3247/946460*100= 0.34
```
- 3503 genes(0.37%) are significant before and after Yogurt
- 3247 genes(0.34%) are significant before and after Yogurt Oatmeal
- only 7 genes are significant in model full


```{r}
sum(PRS_all_Y_sig$feature %in% PRS_all_YO_sig$feature) # 3020
sum(PRS_all_YO_sig$feature %in% PRS_all_Y_sig$feature) # 3020
```




### Comparaison significant genes across models
...
nrow(all_sig_increase) # 3017
nrow(all_sig_decrease) # 3
nrow(onlyY_sig_increase) # 347
nrow(onlyYO_sig_increase) # 128
nrow(onlyY_sig_decrease) # 136
nrow(onlyYO_sig_decrease) # 99
```

- 3020 genes are significant in both Yogurt and Yogurt Oatmeal
  - 3017 increase in both interventions
  - 3 decrease in both interventions
  - none have different direction depending on the intervention
-  483 genes are only significant in Yogurt
  - 347 increase (332 increase, 11 decrease, 4 are NA in YO)
  - 136 decrease (31 increase, 105 decrease, 0 are NA in YO)
-  227 genes are only significant in Yogurt Oatmeal
  - 128 increase (99 increase, 12 decrease, 17 are NA in Y)
  - 99 decrease (19 increase, 80 decrease, 0 are NA in Y)
...
write.xlsx(all_sig_increase, 
           file=paste(path, "SignificantGenes.xlsx", sep=""),
           sheet="all_sig_increase", row.names = F)
write.xlsx(all_sig_decrease, 
           file=paste(path, "SignificantGenes.xlsx", sep=""),
           sheet="all_sig_decrease", append = T, row.names = F)
write.xlsx(onlyY_sig_increase, 
           file=paste(path, "SignificantGenes.xlsx", sep=""),
           sheet="onlyY_sig_increase", append = T, row.names = F)
write.xlsx(onlyYO_sig_increase, 
           file=paste(path, "SignificantGenes.xlsx", sep=""),
           sheet="onlyYO_sig_increase", append = T, row.names = F)
write.xlsx(onlyY_sig_decrease, 
           file=paste(path, "SignificantGenes.xlsx", sep=""),
           sheet="onlyY_sig_decrease", append = T, row.names = F)
write.xlsx(onlyYO_sig_decrease, 
           file=paste(path, "SignificantGenes.xlsx", sep=""),
           sheet="onlyYO_sig_decrease", append = T, row.names = F)
```
...
# 43= 31(sig decrease in Y while nonsig increase in YO) + 12(nonsig decrease in Y while sig increase in YO)
# 30= 11(sig increase in Y while nonsig decrease in YO)  + 19(nonsig increase in Y while sig decrease in YO)
# 188= 3 (sig decrease in both) + 105(sig decrease in Y while non sig decrease in YO) + 80(sig decrease in YO while non sig decrease in Y)
# 3448= 3017 (sig increase in both) + 332(sig increase in Y while non sig increase in YO) + 99(sig increase in YO while non sig increase in Y)
```
