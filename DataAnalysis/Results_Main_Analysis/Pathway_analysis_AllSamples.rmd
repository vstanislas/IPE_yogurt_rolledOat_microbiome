...
path_analysis <- paste(dir, "DataAnalysis/Results_Main_Analysis/ResultsFunctional/On_pathways_AllSamples/", sep="")
...
```{r message=FALSE}
load(paste(dir, "DataPreparation/Microbiome/Functional/TSE_Physeq_Pathway.Rdata", sep=""))
...
```{r}
### UNMAPPED and UNINTEGRATED explain all the variation on first axis
...
**Separation between pathways with or without zeroes made on each subsets.**
...
wd0 <- paste(path_analysis, "ResultsBetadiv/", sep="")
setwd(wd0)
run_taxa_model(path_to_use_w0, package="NBZIMM", counts=F, FE=FE, RE=RE, DF=DF_work_mod, compar_var=compar_var, var_check=NULL, savingName=paste("NBZIMM_asinsqrt_", compa, sep=""), sig_threshold=sig_threshold, check_mod=F, save_mod=F, path="Q:/IPE-P-Joghurtstudie/Joghurt und Haferflocken/Analysis/")
toc()
# Model full: 166.75 sec elapsed
# Model Y: 143.54 sec elapsed / 133.41 sec elapsed
# Model YO: 136.61 sec elapsed / 126.8 sec elapsed

# paste("NBZIMM_asinsqrt/", compa, "/", sep="")
```
