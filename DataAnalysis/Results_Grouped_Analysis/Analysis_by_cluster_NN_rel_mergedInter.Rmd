...
path_results <- paste(dir, "DataAnalysis/Results_Grouped_Analysis/Results_clustering_NN_rel/Inter_merged/", sep="")
...
name_grouping <- "NNGraph_relabundance"
...
ll_plot <- list()
for(i in seq(length(measures))){
  pp <- plot_sample_measure_CO(tse2, measures[i], subtitle[i],  x="Treatment",  print_table=F, trans = "psd_log", line_diff = T) + facet_grid(Intervention ~ group_info) 
  ll_plot[[i]] <- update_lines(pp) 
}

ggsave(paste(path_results, "ResultsAlpha/Alpha_byTreat.png", sep=""), 
       wrap_plots(ll_plot, nrow=3)  + plot_layout(guides = "collect") ,  width=25, height = 12)
```
