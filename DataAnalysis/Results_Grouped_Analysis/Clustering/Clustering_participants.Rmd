...
path_analysis0 <- paste(dir, "DataAnalysis/Results_Main_Analysis/ResultsTaxonomy/", sep="")
path_analysis <- paste(dir, "DataAnalysis/Results_Grouped_Analysis/Clustering/", sep="")
...
```{r}
### Load data with already ordination and alpha diversity 
### contains tse, tse_sub, tse_SGB
load(paste(path_analysis0, "tse_allwithordination.Rdata", sep=""))
```
...
clus$km.cluster_merged[clus$km_T1.cluster==1 & clus$km_T3.cluster==3] <- 1
clus$km.cluster_merged[clus$km_T1.cluster==2 & clus$km_T3.cluster==2] <- 2
clus$km.cluster_merged[clus$km_T1.cluster==3 & clus$km_T3.cluster==1] <- 3
...
part_miss_plot <- grid.arrange(bottom=grid::textGrob(pct_miss_text, gp=grid::gpar(fontsize=9)), tableGrob(part_miss_table, theme=ttheme_minimal(base_size=8, padding = unit(c(4, 2), "mm"))))
```
